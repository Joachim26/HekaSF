...
int randi = rand() % 12 + 1; 
int lopez = std::min(randi, int(bestThread->rootMoves.size())-1);

int suic = 0;

if(Options["Suicide"] && !Options["Random"]){
suic = std::min(200, int(bestThread->rootMoves.size())-1);}

if(!Options["Suicide"] && Options["Random"]){
suic = lopez;}

if((!Options["Suicide"] && !Options["Random"]) || (Options["Suicide"] && Options["Random"])){
suic = 0;}
  ...
  sync_cout << "bestmove " << UCI::move(bestThread->rootMoves[suic].pv[0], rootPos.is_chess960());
...
int oooi = int(Options["tacticalNodes"]) * 100000;
int xlvl = int(Options["tacticalLevel"]) * 10;

  int ncnt = int(Options["NnueContempt"]);
xnco = (us == WHITE ?  make_score(ncnt, ncnt / 2)
                    : -make_score(ncnt, ncnt / 2));
...
kthr = 100;
psqodw = -100;
xnco = (us == WHITE ?  make_score(80, 80 / 2)
                    : -make_score(80, 80 / 2));}
...
int ppa = int(Options["HuntKing"]);
int ppb = int(Options["HuntQueen"]);
int ppc = int(Options["HuntRook"]);
int ppd = int(Options["HuntBishop"]);
int ppe = int(Options["HuntKnight"]);
...
xkn = ppe;
 ...
 int aqq = int(Options["PawnForward"]);
 int fqq = int(Options["AttackBase"]);
 int jqq = int(Options["AttackCentre"]);
 int kqq = int(Options["PawnChains"]);
...
int fmlevel = int(Options["FmpvDifference"]);
int flnumber = int(Options["FmpvMaxMultiPV"]);
...
if(Options["MultiPV"] == 1 && Options["FluidMultiPV"]){
...
else{multiPV = size_t(Options["MultiPV"]);}
...
if(Threads.nodes_searched() >= oooi){
orxp = lvl;}
...
orxp = 0;}
          ...
          && !mainThread->stopOnPonderhit)
      {
          ...
          if (Time.elapsed() > totalTime && Threads.nodes_searched() >= oooi * 2)
          {
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
              ...
              && !givesCheck)
          {
              ...
              if (!pos.see_ge(move, Value(-(29 - std::min(lmrDepth, 18)) * lmrDepth * lmrDepth)) && ss->moveCount >= pos.this_thread()->precb)
                  continue;
          }
          else
          {
              ...
              // See based pruning
              if (!pos.see_ge(move, Value(-221) * depth) && ss->moveCount >= pos.this_thread()->precb) // (~25 Elo)
                  continue;
          }
      }
          ...
          &&  tte->depth() >= depth - 3)
      {
          ...
          // a soft bound.
          else if (singularBeta >= beta && (ss-1)->moveCount >= pos.this_thread()->preca)
              return singularBeta;
      ...
      if (rootNode)
      {
          ...
          if (moveCount == 1 || value > alpha)
          {
              ...
              if (moveCount > 1 && pos.this_thread()->horx == 1)
